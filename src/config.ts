...
const ConfigSchema = z.object({
  ...
  MJU_AGENT_URL: z
    .string()
    .url()
    .default("http://mjuclaw-agent:8000/v1/discord/messages"),
  MJU_AGENT_TIMEOUT_MS: z.coerce.number().int().positive().default(600_000),
  MJU_BIN: z.string().min(1).default("mju"),
  // HTTP 서버: 컨테이너 내부 alert helper(`mju-news-alert`, `mju-attendance-alert`)가
  // agent 측 Discord channel 비활성화에 따라 router로 우회하기 위해 사용한다.
